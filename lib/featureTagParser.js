...
class FeatureTagParser {
  ...
  async getFeatureTags(filePath) {
    ...
    try {
      ...
      for (const line of lines) {
        ...
        // If we hit an empty line after we've started collecting tags, reset them
        // This ensures tags must be directly above Feature: without blank lines
        if (!trimmed) {
          if (tags.length > 0) {
            tags = [];
          }
        ...
        if (trimmed.startsWith('@')) {
          ...
          // Reset tags if we encounter non-tag, non-comment content before Feature:
          // This ensures we only get tags directly above Feature:
          tags = [];
        }
      }
  ...
  parseExpression(expression) {
    ...
    // Handle "and not" pattern with proper parentheses support
    const andNotMatch = expression.match(/^(.+?)\s+and\s+not\s+(.+)$/i);
    if (andNotMatch) {
      const rightPart = andNotMatch[2].trim();
      // Check if the right part starts with parentheses
      if (rightPart.startsWith('(')) {
        // Find matching closing parenthesis
        let depth = 0;
        let closeIndex = -1;
        for (let i = 0; i < rightPart.length; i++) {
          if (rightPart[i] === '(') depth++;
          if (rightPart[i] === ')') {
            depth--;
            if (depth === 0) {
              closeIndex = i;
        ...
        if (closeIndex > 0 && closeIndex === rightPart.length - 1) {
          // The entire right part is in parentheses
          return {
            type: 'and',
            left: this.parseExpression(andNotMatch[1]),
            right: {
              type: 'not',
              operand: this.parseExpression(rightPart)
            }
          };
        }
      }
      return {
        ...
        right: {
          type: 'not',
          operand: this.parseExpression(andNotMatch[2])
        }
      };
    }
  ...
  evaluateExpression(expr, tags) {
    ...
    switch (expr.type) {
      case 'tag':
        return tags.includes(expr.value);
